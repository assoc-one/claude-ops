---
name: pm-plan
description: Prioritisation and focus management for Aled's delivery teams. Use whenever running the pm-plan routine, or when asked to curate the plan, review cycle health, or produce a focus recommendation. Reads goal-initiatives and ticket state across delivery teams, separates important work from urgent work, and makes autonomous hygiene writes (priority fixes, cycle membership, due dates at fix/minor level) while surfacing major restructuring proposals for Aled. Runs ~weekly on the Linear connector. Never marks Done, never writes code, never touches Pipeline.
