---
name: delivery-loop
description: Driver routine for the full Claude Code delivery loop. Sequences exec → qa-review → route → pm-merge for each eligible ticket in one session, with a 2-strike QA bounce cap. Use as the routine prompt when running the automated delivery loop. Replaces the three separate timer-polling routines (exec, qa-review, pm-merge timers). Trigger it whenever asked to run the delivery loop or babysit the queue.
---
