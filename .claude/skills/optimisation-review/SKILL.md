...
description: Weekly review skill that audits how Aled's tooling is used and how workflows are shaped, then raises improvement proposals into the os.Claude Backlog. Two passes — tooling (are we using Claude, Linear, GitHub, Vercel, Sanity well, per current best practice?) and workflow (does the operating shape match the skill-vs-task test?). Read-only to canon; every finding becomes a Backlog ticket for Aled to approve. Never edits a SKILL.md, routine, or other canon directly.
---
