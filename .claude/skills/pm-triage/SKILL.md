---
name: pm-triage
description: Intake of new Backlog cc-pm tickets in Aled's delivery projects. Use this whenever running the pm-triage routine, or when a new ticket in Backlog carries the agent:cc-pm label — shape it to standard, flag gaps, and move it to Refinement. Does not process Refinement comments, Todo routing, send-back bounces, or Blocked sweeps; those belong to pm-coordinate. Trigger it whenever the task is to triage or intake a new Backlog ticket in a Linear context.
---
